    ...
    for x in range(inc.MAZELENGTH):
        for y in range(inc.MAZEWIDTH):
            for d in range(inc.DIRECTIONS):
                newy = y + inc.dy[d]
                newx = x + inc.dx[d]
                if (0<=newx<inc.MAZELENGTH and 0<=newy<inc.MAZEWIDTH):
                    maze[x][y].succ[d] = maze[newx][newy]
                else:
                    maze[x][y].succ[d] = None
    # #ifdef DEBUG
    #     assert(STARTY % 2 == 0);
    #     assert(STARTX % 2 == 0);
    #     assert(GOALY % 2 == 0);
    ...
    for x in range(inc.MAZELENGTH):
        for y in range (inc.MAZEWIDTH):
            for d1 in range(inc.DIRECTIONS):
                maze[x][y].move[d1] = maze[x][y].succ[d1]
...
def newrandommaze():
    ...
    obstWidth = 10
    for i in range(1):
        obstCentrePoint.append((random.randint(int(0.1*inc.MAZELENGTH),int(0.8*inc.MAZELENGTH)), random.randint(int(0.1*inc.MAZEWIDTH),int(0.8*inc.MAZEWIDTH))))
        for x in range(obstCentrePoint[i][0]-obstLength/2, obstCentrePoint[i][0]+obstLength/2):
            maze[x][obstCentrePoint[i][1]-obstWidth/2]. obstacle = 1
            maze[x][obstCentrePoint[i][1]+obstWidth/2]. obstacle = 1
        for y in range(obstCentrePoint[i][1]-obstWidth/2, obstCentrePoint[i][1]+obstWidth/2):
            maze[obstCentrePoint[i][0]-obstLength/2][y].obstacle = 1
            maze[obstCentrePoint[i][0]+obstLength/2][y].obstacle = 1
    # for y in range (inc.MAZEWIDTH):
    #     for x in range(inc.MAZELENGTH):
