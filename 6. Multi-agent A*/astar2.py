...
def generateRoughTrajectory():
    ...
    f = open(experiment_time+'_UAV2_execT', 'a')
    f.write('roughPath\t'+str(execution_time)+'\n')
    f.close()

    f = open(experiment_time+'_UAV2_rufPath', 'a')
    ...
    f = open(experiment_time+'_UAV2_pathL', 'a')
    f.write('roughPath\t'+str(len_of_path/scale_rough)+'\n')
    f.close()

    f = open(experiment_time+'_UAV2_heapPerc', 'a')
...
startPoint  = (0, 0, 0)
endPoint    = (0, 0, 0)
current_position  = (1.0, 3.5, 1.0)
target_position   = (3.5, 1.0, 1.5)
...
while callback_current_flg:
    ppSub   = rospy.Subscriber('/UAV_2/pose', PoseStamped, callback_current)
